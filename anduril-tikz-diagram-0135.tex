...
\newcommand{\spacingv}{\\[0.75ex]} % for vertical spacing
...
\tikzset{
    rampstepped/.style={ % for ramp image
        rectangle,
        draw=none,
...
\tikzset{
    anyline/.style={ % any = for Simple UI and Advanced UI
        draw=black,
        thin
    },
...
\tikzset{
    anyline-mode/.style={ % mode = for strobe modes and blinky modes
        anyline
    },
...
\tikzset{
    advancedline/.style={ % advanced = for Advanced UI only
        anyline
    },
...
\tikzset{
    simpleline/.style={ % simple = for Simple UI only
        anyline
    },
...
\tikzset{
    extendedsimpleline/.style={ % extendedsimple =
    % for Extended Simple UI and Advanced UI
        anyline
    },
...
\setlength{\tabcolsep}{0pt} % for setting the table column spacing
% default: 6pt
...
\newcommand{\infotextextendedsimple}[1]{\textcolor{black}{#1}}
% extendedsimple = for infotext for Extended Simple UI and Advanced UI

\newcommand{\inherited}{\char"002A}
% for Unicode character Asterisk
\newcommand{\default}{\char"25C1}
% for Unicode character White Left-Pointing Triangle
...
{\infotextextendedsimple{Smooth\default{}}};
\node (Ramp Style Stepped)[
    below left=0.05cm and 0.3cm+0.02cm of Ramp.south east,
    % +0.02cm for vertical alignment
    anchor=north,
    align=center
]
{\infotextextendedsimple{Stepped}};
\node (Ramp Styles fit)[
    inner sep=-0.05cm,
...
\begin{scope}[on background layer]
    ...
    at (Turbo.north) {\oneline{\infotextadvanced{Turbo}}};
    \node (Ramp label)[
        on-extra,
        inner xsep=0.425cm,
        above,
...
\node (Toggle Ramp Style extra node ON)[
    on-extra,
    below right=0.85cm and 0.1cm of Toggle Ramp Style from ON.west,
    anchor=center
]
{ON};
...
node[
    ...
    label={
        [align=left, xshift=-0.05cm, yshift=-0.025cm]right:{
            \begin{tabular}{l}
                \infotextextendedsimple{3C if only one}\\
                \infotextextendedsimple{channel mode}
            \end{tabular}
        }
    }
]
...
\node (Candle from OFF)[
    extendedsimpleconfig-mode,
    above right=0.45cm and 1.2cm of Lightning Storm from Police Strobe.east,
    anchor=west
]
...
\node (Candle Timer)[
    extendedsimpleconfig,
    above left=0.66cm and 1cm of Candle from OFF.west,
    anchor=east
]
{
...
\begin{scope}[on background layer]
    ...
    {
        \normalsize\textbf{\infotextextendedsimple{Strobe / Mood Modes}}
        \spacingv
        \infotextextendedsimple{1C = Off}
    };
\end{scope}
...
\begin{scope}[on glass layer]
    ...
    {3H} (Strobe Modes fit.44);
\end{scope}

\draw (Strobe Modes fit.61) edge[
    ->,
    extendedsimpleline,
    out=90,
    in=180,
...
node[
    ...
    label={
        [align=left, xshift=-0.05cm, yshift=-0.025cm]right:{
            \begin{tabular}{l}
                \infotextextendedsimple{Momentary}\\
                \infotextextendedsimple{Strobe Mode}
            \end{tabular}
        }
   }
]
...
\begin{scope}[on glass layer]
    \draw (OFF.south) edge[
        {Round Cap[sep=6.9cm]}-{Round Cap[sep=3cm]},
        anyline,
        white,
        line width=2.4pt,
        out=270,
...
\begin{scope}[on background layer]
    \node (Voltage Config from Batt Check)[
        advancedtable,
        above right=0.4cm and 1.4cm of Tactical Mode Config.south east,
        anchor=south west
    ]
    {
...
\node (Channel Mode Config from ON)[
    advancedtable,
    above=0.25cm of Next Channel Mode from ON.north west,
    anchor=south west
]
{
    \begin{tabular}{m{0.35cm} l}
...
{10H} (Ramp Extras Config from ON.268);
...
]
{
    ...
    \normalsize Diagram Release 0135-39\\[1.5ex]
    Diagram Repository:\\
    https://github.com/dirtydancing/\\
    anduril-tikz-diagram-0135
    \spacingv
