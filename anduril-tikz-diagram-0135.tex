...
\node (Auto-lock Config from Lockout)[ 
    advancedconfig,
    left=1.5cm of Aux LED Pattern from Lockout.north west,
    anchor=north east
]
{
    \firstline{Auto-lock*}
...
% Channel Mode Config from Lockout
\node (Channel Mode Config from Lockout)[
    advancedconfig,
    left=0.4cm of Auto-lock Config from Lockout.north west,
    anchor=north east
]
{
    \oneline{Channel Mode}\\[0.75ex]
    \firstline{Config*}
    See 9H from On
};
...
{9H} (Channel Mode Config from Lockout.325);
...
]
{
    ...
    \normalsize Diagram Release 0135-14\\[2.0ex]
    Diagram Repository:\\
