...
\node (Candle Timer)[
    extendedsimpleconfig,
    above left=0.67cm and 1cm of Candle from OFF.west,
    anchor=east
]
{
...
\begin{scope}[on background layer]
    ...
    {
        \normalsize\textbf{Strobe / Mood Modes}\\[1.0ex]
        1C = Off
    };
\end{scope}
...
\draw (OFF.south) edge[
    {Round Cap[sep=6.9cm]}-{Round Cap[sep=3.1cm]},
    anyline,
    white,
    line width=2.4pt,
    out=270,
...
\begin{scope}[on background layer]
    ...
    {
        \normalsize\textbf{Blinky / Utility Modes}\\[1.0ex]
        1C = Off
    };
\end{scope}
...
{9H} (Channel Mode Config from ON.322);
...
]
{
    ...
    \voltagebox[cyan]{a\vphantom{Voltage}}%
    \voltagebox[blue]{g\vphantom{Voltage}}%
    \voltageboxpurple{e\vphantom{Voltage}}\default{}
};
...
]
{
    ...
    \normalsize Diagram Release 0135-36\\[2.0ex]
    Diagram Repository:\\
...
{
    \oneline{Aux LED Color for Voltage}\\[0.75ex]
